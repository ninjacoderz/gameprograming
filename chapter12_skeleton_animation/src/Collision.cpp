...
float LineSegment::MinDistSq(const LineSegment & s1, const LineSegment & s2)
{
	...
	if (GameMath::NearZero(D)) { // the lines are almost parallel
	...
	sc = (GameMath::NearZero(sN) ? 0.0f : sN / sD);
	tc = (GameMath::NearZero(tN) ? 0.0f : tN / tD);
...
{
    mMin.x = GameMath::Min(mMin.x, point.x);
    mMin.y = GameMath::Min(mMin.y, point.y);
    mMin.z = GameMath::Min(mMin.z, point.z);

    mMax.x = GameMath::Max(mMax.x, point.x);
    mMax.y = GameMath::Max(mMax.y, point.y);
    mMax.z = GameMath::Max(mMax.z, point.z);
}
...
bool ConvexPolygon::Contains(const Vector2& point) const
{
	...
	for (size_t i = 0; i < mVertices.size() - 1; i++)
	{
		...
		b.Normalize();
		// Add angle to sum
		sum += GameMath::Acos(Vector2::Dot(a, b));
	}
	...
	return GameMath::NearZero(sum - GameMath::TwoPi);
}
...
float AABB::MinDistSq(const Vector3& point) const
{
	...
	float dx = GameMath::Max(mMin.x - point.x, 0.0f);
	dx = GameMath::Max(dx, point.x - mMax.x);
	float dy = GameMath::Max(mMin.y - point.y, 0.0f);
	dy = GameMath::Max(dy, point.y - mMax.y);
	float dz = GameMath::Max(mMin.z - point.z, 0.0f);
	dz = GameMath::Max(dy, point.z - mMax.z);
...
bool Intersect(const LineSegment& l, const Plane& p, float& outT)
{
	...
	float denom = Vector3::Dot(l.mEnd - l.mStart,
		p.mNormal);
	if (GameMath::NearZero(denom))
	{
		// The only way they intersect is if start
		// is a point on the plane (P dot N) == d
		if (GameMath::NearZero(Vector3::Dot(l.mStart, p.mNormal) 
			- p.mD))
		{
			return true;
		}
